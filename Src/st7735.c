...
{
  nrf_gpio_pin_clear(ST7735_CS_Pin);
}

void ST7735_Unselect()
{
  nrf_gpio_pin_set(ST7735_CS_Pin);
}

static void ST7735_Reset()
{
  nrf_gpio_pin_clear(ST7735_RES_Pin);
  nrf_delay_ms(5);
  nrf_gpio_pin_set(ST7735_RES_Pin);
}

static void ST7735_WriteCommand(uint8_t cmd)
{
    nrf_gpio_pin_clear(ST7735_DC_Pin);
    ret_code_t err_code = nrf_drv_spi_transfer(&spi, &cmd, sizeof(cmd), NULL, 0);
...
static void ST7735_WriteData(uint8_t *buff, size_t buff_size)
{
  ...
  ret_code_t err_code = nrf_drv_spi_transfer(&spi, buff, buff_size, NULL, 0);
  APP_ERROR_CHECK(err_code);
}
// implement this instead of having it written out in functions
void sendSPI(uint8_t *data, int size)
{
    ret_code_t err_code = nrf_drv_spi_transfer(&spi, data, size, NULL, 0);
    APP_ERROR_CHECK(err_code);
}
